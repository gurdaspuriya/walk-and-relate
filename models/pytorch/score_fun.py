...
class TransEScore(nn.Module):
    ...
    def prepare_path_embedding(self, relation_embedding, direction_trace, path_len, gpu_id):
        
#         direction_trace = direction_trace[:,:1]
#         relation_embedding = relation_embedding[:,:1]
        
        maxlen = direction_trace.size(1)
        mask_arange = th.arange(maxlen)[None, :]
        if gpu_id >= 0:
            direction_trace = direction_trace.cuda(gpu_id)
            path_len = path_len.cuda(gpu_id)
            mask_arange = mask_arange.cuda(gpu_id)
            
        mask = mask_arange < path_len[:, None]
        direction_trace[~mask] = 0
        direction_trace = direction_trace.float().unsqueeze(-1).expand_as(relation_embedding)
#         print(direction_trace.size())
#         print((direction_trace*relation_embedding).size())
#         print(relation_embedding, direction_trace)
#         print((direction_trace*relation_embedding))
#         print(th.sum(direction_trace*relation_embedding, dim=1))
#         print(th.linalg.norm(relation_embedding[:,0,:] - th.sum(direction_trace*relation_embedding, dim=1), dim=1), direction_trace)
#         return relation_embedding[:,0,:]
        return th.sum(direction_trace*relation_embedding, dim=1)
...
class TransRScore(nn.Module):
    ...
    def update(self, weight_decay = 0.0, norm_p = 3, gpu_id=-1):
        self.projection_emb.update(weight_decay, norm_p, gpu_id)
