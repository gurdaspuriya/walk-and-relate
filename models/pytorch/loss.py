...
class LossGenerator(BaseLossGenerator):
    ...
    def get_total_loss(self, pos_score, neg_score, edge_weight=None):
        ...
        if edge_weight is None:
            edge_weight = th.ones(len(pos_score)).to(pos_score.device)
        if self.pairwise:
        ...
        neg_loss = self._get_neg_loss(neg_score) * edge_weight.unsqueeze(-1) # we don't use edge weight for negative samples, don't see the logic
...
#         neg_loss = th.mean(neg_loss)*th.sum(edge_weight)/len(pos_score)
#         pos_loss = th.mean(pos_loss)*1.5
        neg_loss = th.sum(neg_loss)/th.sum(edge_weight)
        pos_loss = th.sum(pos_loss)/th.sum(edge_weight)
